...
Instead of manually extracting and saving, plugins can push files directly into the application's global background queue. This automatically handles OCR extraction, parsing, saving, and showing UI toasts.

```python
# Import a file into the processing queue
success = self.api.processing.import_file_to_queue(
    file_path="C:/path/to/invoice.pdf",
    source="my_plugin",
    metadata={"sender": "john_doe"} # Optional metadata attached to this import
)
```
...
### Event Subscriptions (Advanced)

Plugins can subscribe to global processing events to implement automated feedback (e.g., replying to a user when their document fails).

```python
def on_load(self):
    # Subscribe to events
    self.api.processing.on_duplicate(self._handle_duplicate)
    self.api.processing.on_processing_failed(self._handle_failure)
    return True

def _handle_duplicate(self, file_path, existing_data, source, metadata):
    # Only react to events generated by THIS plugin
    if source != self.id:
        return
    print(f"Duplicate detected! Original Invoice: {existing_data.get('invoice_number')}")

def _handle_failure(self, file_path, error, source, metadata):
    if source != self.id:
        return
...
| Import error         | Add package to `dependencies` **and** bundle it in `libs/` for frozen builds |
